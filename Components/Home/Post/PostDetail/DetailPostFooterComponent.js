...
const DetailPostFooterComponent = ({ post,setPost }) => {
  ...
  const FechLike = async (auth) => {
    ...
    if (!post.liked) {
      try {
        const result = await axios.post(`${baseUrl}/postlike/`, {
            post: post.id,
            user: user.id,
          },{headers},
        );
        setPost(prev=>({...prev,liked:!prev.liked,number_of_likes:prev.number_of_likes+1}))
      } catch (error) {
        setLikeError(error);
      }
    } else {
      try {
        const result = await axios.get(`${baseUrl}/postlike/?ordering=-date&user=${user.id}&post=${post.id}`);
        if(result){
          try {
            axios.put(`${baseUrl}/postlike/${result.data[0].id}/`,{
                un_liked: true,
