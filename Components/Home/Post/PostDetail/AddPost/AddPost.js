...
const AddPost = () => {
  ...
  const handleKeyPress = ({ nativeEvent }) => {
    if (nativeEvent.key == "Escape" || nativeEvent.key == "Backspace") {
      ...
      if (
        focusedFieldIndex == fragmetns.length - 1 &&
        fragmetns[focusedFieldIndex - 1]?.type != "FT"
      )
        return;
  ...
  const mergeFtFragments = (unMergeFragments) => {
    ...
    let mergeFragmentsList = unMergeFragments.filter((fragment, index) => {
      if (
        index - 1 >= 0 &&
        fragment.type == "FT" &&
        resultList[index - 1].type == "FT"
      ) {
        resultList[index - 1].value =
          resultList[index - 1].value + fragment.value;
    ...
    setFragmetns((prev) => {
      ...
      while (true) {
        ...
        if (fragmetnLength == 0) {
          ...
          if (
            newFragment.length - 1 == index &&
            newFragment[index - 1]?.type != "FT"
          )
            break;
          console.log(
            newFragment.length - 1 == index,
  ...
  const addSubTitleHandler = () => {
    ...
    const valueLengthOfFocusedField =
      fragmetns[focusedFieldIndex]?.value?.length;

    if (
      fragmetns[focusedFieldIndex]?.type == "FT" &&
      cursorPosition != 0 &&
      valueLengthOfFocusedField != cursorPosition
    ) {
    ...
    setFragmetns((prev) => {
      ...
      let spliceFragments = [
        spliceFragmentSubtitleComponenet,
        spliceFragmentFirstComponenet,
      ];

      newFragment.splice(nextIndex, 0, ...spliceFragments);
      newFragment.map((frag, index) =>
        console.log(frag.value, index, "spliced value")
      );
  ...
  const postToApiHandler = async () => {
    ...
    try {
      setError(false);
      setLoading(true);
      const result = await axios.post(`${baseUrl}/post/`, postFullData, {
        headers,
      });
  ...
  return (
    ...
    >
      <View style={{ flex: 1 }}>
        ...
        {error == "TitleError" && <ErrorText error={"Title Field Required"} />}
        {error == "ServerError" && (
