...
const Calender = () => {
  ...
  const {darkMode} = useContext(DarkModeContext)
    ...
    const listDaysInMonth = (year,month)=>{
      ...
      for(i=1;i<=30;i++){
        ...
        let savedSchedule = schedules.filter(schedule=> schedule.sh_date == `${year}-${month}-${i}`)
        if(savedSchedule.length >0){
          listofDaysInAmonth.push({ethiopiandate:[year,month,i],gregorianDate,schedule:savedSchedule})
          console.log(savedSchedule,'saved schedule')
        }else{
          listofDaysInAmonth.push({ethiopiandate:[year,month,i],gregorianDate})
        }
      }
  ...
  return (
    <View style ={{width:"100%",alignItems:'center',backgroundColor:darkMode?color.darkBackground:'red',padding:1}}>
    ...
    { listOfDays && <FlatList 
      ...
      style={{backgroundColor:darkMode?color.darkBackground:'white',margin:0}}
      onRefresh={refreshHandler}

      ListHeaderComponent={()=>(<>
      <HeaderCalender year={year} month={month} setMonth={setMonth} setYear={setYear}/>
      </>)}
