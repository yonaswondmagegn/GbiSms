import { StyleSheet, Text, View,Switch,Image,ScrollView,ImageBackground} from 'react-native'
...
<View style = {styles.profileDiscContainer}>
    ...
    <Text style = {styles.usernameText}>{profileData?.user?.username} {profileData?.user?.last_name}</Text>
   ...
   <View style = {styles.acadamicStatusContainer}>
    ...
    <View style={styles.academicInfoCont}>
        ...
        <Text style={{color:'whitesmoke',marginLeft:25}}>{acadamic_year?acadamic_year:'AcademicYear Not Found'}</Text>
       <Text style={{color:'whitesmoke',marginLeft:55}}>{(profileData?.major?.title) ?(profileData?.major?.title):'Department Not Found'}</Text>
    </View>
    <View style = {styles.kiflatContainer}>
        ...
        {profileData?.groupNames?.length >0 ? profileData?.groupNames.map(name=><Text style={{color:'whitesmoke',marginLeft:30}} key={name}> #{name}</Text>):<Text style={{color:'whitesmoke',marginLeft:30}}># ክፍላት ዉስጥ አልተገኘም ።</Text>}
    </View>
</View>
</View>
  return (
    ...
    <>
    {profileData && <ProfilePost footerFragemnt={footerFragemnt} profile={profileData}/> }
    </>
